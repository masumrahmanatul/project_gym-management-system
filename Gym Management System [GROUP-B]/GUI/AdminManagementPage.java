...
public class AdminManagementPage extends JFrame implements ActionListener{
	...
	Font font23 = new Font("Agency FB", Font.BOLD, 23);
	Font font20 = new Font("Agency FB", Font.BOLD, 20);
	
	AdministratorList administratorList = new AdministratorList(1000);
	...
	JTextField idField, nameField, ageField, genderField, searchField, deleteField;
	JComboBox<String> roleComboBox;
	JButton addButton, searchButton, deleteButton, showAllButton, clearScreenButton, updateButton, backButton;
	...
	public AdminManagementPage(){
		...
		FileIO.loadAdminFromFile(administratorList);
		...
		backButton.setBounds(35, 20, 100, 30);
		backButton.setBackground(Color.ORANGE);
		backButton.setFont(font23);
		this.add(backButton);
		...
		subTitle = new JLabel("ADMIN INFORMATIONS");
		...
		screen.setText(administratorList.getAllAsString());
		...
		nameLabel.setBounds(490, 170, 100, 30);
		nameLabel.setForeground(Color.WHITE);
		nameLabel.setFont(font20);
		this.add(nameLabel);
		
		nameField = new JTextField();
		nameField.setBounds(490, 200, 150, 30);
		...
		ageLabel.setBounds(490, 250, 100, 30);
		ageLabel.setForeground(Color.WHITE);
		ageLabel.setFont(font20);
		this.add(ageLabel);
		
		ageField = new JTextField();
		ageField.setBounds(490, 280, 150, 30);
		...
		genderLabel.setBounds(490, 330, 100, 30);
		genderLabel.setForeground(Color.WHITE);
		genderLabel.setFont(font20);
		this.add(genderLabel);
		
		genderField = new JTextField();
		genderField.setBounds(490, 360, 150, 30);
		...
		roleLabel.setBounds(490, 410, 100, 30);
		roleLabel.setForeground(Color.WHITE);
		roleLabel.setFont(font20);
		this.add(roleLabel);
		
		String[] roles = {"Membership Administrator", "Financial Manager", 
		"Scheduling Coordinator", "Maintenance Administrator"};
		roleComboBox = new JComboBox<>(roles);
		roleComboBox.setBounds(490, 440, 170, 30);
		roleComboBox.setFont(font20);
		this.add(roleComboBox);
		
		addButton = new JButton("ADD");
		addButton.setBounds(490, 490, 150, 40);
		...
		updateButton.setBounds(490, 550, 150, 40);
		...
		searchLabel.setBounds(720, 90, 100, 30);
		searchLabel.setForeground(Color.WHITE);
		searchLabel.setFont(font20);
		this.add(searchLabel);
		
		searchField = new JTextField();
		searchField.setBounds(720, 120, 150, 30);
		...
		searchButton.setBounds(720, 160, 150, 40);
		...
		deleteLabel.setBounds(720, 220, 100, 30);
		deleteLabel.setForeground(Color.WHITE);
		deleteLabel.setFont(font20);
		this.add(deleteLabel);
		
		deleteField = new JTextField();
		deleteField.setBounds(720, 250, 150, 30);
		...
		deleteButton.setBounds(720, 290, 150, 40);
		...
		imageLabel.setBounds(720, 320, 180, 180);
		ImageIcon image = new ImageIcon("strength_factory.png");
		imageLabel.setIcon(image);
		this.add(imageLabel);
		
		showAllButton = new JButton("SHOW ALL");
		showAllButton.setBounds(720, 490, 150, 40);
	...
	public void actionPerformed(ActionEvent e){
		if(addButton == e.getSource()){
			...
			if(a==null){
				if(!id.isEmpty() && name.length()>=2 && !role.isEmpty()){
					Administrator newAdmin = new Administrator(name, age, gender, role, id);
					administratorList.insert(newAdmin);
					FileIO.saveAdmin(newAdmin);
					screen.setText(administratorList.getAllAsString());
				}
				else{
			...
			Administrator a = administratorList.getById(idField.getText());
			if(a!=null){
				...
				JOptionPane.showMessageDialog(this, "No Admin Found with this ID", "WARNING", JOptionPane.WARNING_MESSAGE);
			}
		}
		else if(searchButton == e.getSource()){
			System.out.println("You've clicked the Search Button!");
			Administrator a = administratorList.getById(searchField.getText());
			if(a!=null){
				screen.setText(a.getAdminAsString());
			}
			else{
				JOptionPane.showMessageDialog(this, "No Admin Found with this ID", "WARNING", JOptionPane.WARNING_MESSAGE);
			}
			...
			Administrator a = administratorList.getById(deleteField.getText());
			if(a!=null){
				int option = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete?");
				if(option == JOptionPane.YES_OPTION){
					administratorList.deleteById(deleteField.getText());
					screen.setText(administratorList.getAllAsString());
				}
				else{
					JOptionPane.showMessageDialog(this, "No Admin Found with this ID");
				}
			}
		}
