...
public class StaffManagementPage extends JFrame implements ActionListener{
	...
	JTextArea screen;
	JLabel idLabel, nameLabel, ageLabel, genderLabel, roleLabel, searchLabel,
	deleteLabel, imageLabel;
	JTextField idField, nameField, ageField, genderField, roleField, searchField,
	deleteField;
	...
	public StaffManagementPage(){
		...
		FileIO.loadStaffFromFile(staffList);
		...
		subTitle = new JLabel("STAFF INFORMATIONS");
		...
		screen.setText(staffList.getAllAsString());
		...
		roleField.setBounds(490, 440, 150, 30);
	...
	public void actionPerformed(ActionEvent e){
		if(addButton == e.getSource()){
			...
			Staff a = staffList.getById(id);
			if(a==null){
				if(!id.isEmpty() && name.length()>=2 && !role.isEmpty()){
					Staff newStaff = new Staff(name, age, gender, role, id);
					staffList.insert(newStaff);
					FileIO.saveStaff(newStaff);
					screen.setText(staffList.getAllAsString());
				}
				else{
			...
			Staff a = staffList.getById(idField.getText());
			if(a!=null){
				...
				JOptionPane.showMessageDialog(this, "No Staff Found with this ID", "WARNING", JOptionPane.WARNING_MESSAGE);
			}
		}
		else if(searchButton == e.getSource()){
			System.out.println("You've clicked the Search Button!");
			Staff a = staffList.getById(searchField.getText());
			if(a!=null){
				screen.setText(a.getStaffAsString());
			}
			else{
				JOptionPane.showMessageDialog(this, "No Staff Found with this ID", "WARNING", JOptionPane.WARNING_MESSAGE);
			}
			...
			Staff a = staffList.getById(deleteField.getText());
			if(a!=null){
				int option = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete?");
				if(option == JOptionPane.YES_OPTION){
					staffList.deleteById(deleteField.getText());
					screen.setText(staffList.getAllAsString());
				}
				else{
					JOptionPane.showMessageDialog(this, "No Staff Found with this ID");
				}
			}
		}
